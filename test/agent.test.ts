...
test('CLI /init command initializes agent', async t => {
	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'gilfoyle-agent-test-'));
...
test('CLI shows initialization option', async t => {
	...
	t.true(result.stdout.includes('/init') || result.stdout.includes('Initialize'));
});
...
test('CLI shows chat option in menu', async t => {
	...
	t.true(result.stdout.includes('conversing') || result.stdout.includes('AI agent'));
});
...
test('CLI shows compact option', async t => {
	...
	t.true(result.stdout.includes('/compact') || result.stdout.includes('Compact'));
});
