...
async function demonstrateCLITesting() {
  ...
  log('2. Configuration Tests - Config loading, validation, persistence', colors.reset);
  log('3. Agent & Tool Tests - Initialization, chat, export, workflows', colors.reset);
  log('4. Stress Tests - Performance, security, edge cases, cleanup', colors.reset);
  ...
  log('\nnode test.cjs help      # Show detailed test runner help', colors.reset);
  ...
  log('4. Validate output: stdout.includes("Help - Available Commands")', colors.reset);
