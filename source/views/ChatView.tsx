...
function truncateMessage(message: string, maxLines: number = 10): {content: string, needsTruncation: boolean} {
...
export default function ChatView() {
	...
	const processedHistory: HistoryMessage[] = state.history.map((item: string, index: number) => {
	...
	return (
		<Box flexDirection="column" marginY={1}>
			...
			{processedHistory.length > 0 && (
				<Box flexDirection="column" marginY={1}>
					...
					<Box flexDirection="column" marginLeft={1} minHeight={5}>
						{processedHistory.slice(-9).map((msg: HistoryMessage, index: number) => (
			...
			<Box>
				<Text color="gray">Press ESC to return home • Press Ctrl+E to expand/collapse latest long message</Text>
			</Box>
		</Box>
	);
}
